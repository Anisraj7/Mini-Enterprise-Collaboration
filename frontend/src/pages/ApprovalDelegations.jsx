...
export default function ApprovalDelegations() {
  ...
  const fetchDelegations = async () => {
    try {
      ...
      const mineRows = Array.isArray(mine.data)
        ? mine.data
        : mine.data?.items || [];

      const activeRows = Array.isArray(active.data)
        ? active.data
        : active.data?.items || [];

      const byId = new Map(
        [...mineRows, ...activeRows].map((item) => [item.id, item]),
      );
      ...
      setError(
        err.response?.data?.detail || "Unable to load approval delegations.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const delegationRows = Array.isArray(delegations)
  ? delegations
  : [];
  ...
  const columns = [
...

    {
      ...
      header: "Status",
      render: (row) => (
        <StatusBadge status={row.is_active ? "Active" : "Cancelled"} />
      ),
    },
    ...
    <div className="min-h-screen bg-gray-100">
      <main className="mx-auto max-w-7xl p-6">
            ...
            >
              ...
              <div className="space-y-4">
                <UserSelectDropdown
                  ...
                  onChange={(value) =>
                    setForm({ ...form, delegatee_id: value })
                  }
                  roles={["organization_admin", "workspace_admin", "manager"]}
                  className="w-full"
                />
                <input
                  type="datetime-local"
                ...
                <textarea
                  ...
                  className="h-28 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"
                />
              </div>
        ...
        <ConfirmModal
          isOpen={modal === "cancel"}
          title="Cancel Delegation"
          message="Cancel this approval delegation?"
          onConfirm={cancelDelegation}
