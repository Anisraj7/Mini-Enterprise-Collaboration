...
export default function AuditLogs() {
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    const afterStart =
      !startDate || (created && created >= new Date(`${startDate}T00:00:00`));
    const beforeEnd =
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <main className="mx-auto max-w-7xl p-6">
        <PageHeader
          title="Audit Logs"
          subtitle="Review backend activity with module, user, request, and data snapshot details"
        />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="max-h-[85vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <pre className="rounded-lg bg-gray-50 p-4 text-xs text-gray-700 whitespace-pre-wrap">
                  Old Data: {selectedLog.old_data || "N/A"}
                </pre>
                <pre className="rounded-lg bg-gray-50 p-4 text-xs text-gray-700 whitespace-pre-wrap">
                  New Data: {selectedLog.new_data || "N/A"}
                </pre>
              </div>
